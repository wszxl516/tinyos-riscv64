...
use crate::task::{current_task_pid, set_task_state_by_pid, State};
...
pub fn syscall(regs: &mut Regs) {
    match SyscallNo::from_value(regs.context.a7) {
        SyscallNo::Sleep => match current_task_pid() {
            Some(pid) => {
                set_task_state_by_pid(pid, State::Sleeping);
                set_soft_timer(regs.context.a0 as u64, pid);
                task_switch(&mut regs.context);
                regs.context.a0 = 0
            }
            None => regs.context.a0 = 1,
        },
