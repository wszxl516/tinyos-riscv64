...
pub fn find(vendor: u16, device: u16) -> (&'static str, &'static str) {
    ...
    for line in PCI_IDS.lines() {
        write!(buffer, "{:04x}", vendor).unwrap();
        if line.starts_with(buffer.as_str()) {
            vendor_name = line.split(PLACE_SYMBOL).last().unwrap();
            device_name = "";
            continue;
        }
        buffer.clear();
        write!(buffer, "{}{:04x}", PLACE_SYMBOL, device).unwrap();
        if line.starts_with(buffer.as_str()) {
            device_name = line.split(PLACE_SYMBOL).last().unwrap();
            break;
        }
        buffer.clear();
    }
